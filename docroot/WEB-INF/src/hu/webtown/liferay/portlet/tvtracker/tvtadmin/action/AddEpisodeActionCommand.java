...
public class AddEpisodeActionCommand extends BaseActionCommand{

	private Log _logger = LogFactoryUtil.getLog(AddEpisodeActionCommand.class); 
	...
	protected void doProcessCommand(PortletRequest portletRequest, PortletResponse portletResponse) 
		...
		try {
			...
			Episode createdEpisode = addEpisode(portletRequest, portletResponse);
			
			portletRequest.setAttribute(CustomWebKeys.EPISODE, createdEpisode);
			...
			SessionMessages.add(portletRequest, "add-episode-successful");
			
			if (_logger.isDebugEnabled()) {
				
				long episodeId = createdEpisode.getEpisodeId();
				String episodeTitle = createdEpisode.getEpisodeTitle();
				
				_logger.debug("Episode: (id: " + episodeId + ", title: " + episodeTitle + ") creating was successful!");
			}
	...
	protected Episode addEpisode(PortletRequest portletRequest, PortletResponse portletResponse) 
			throws Exception {
